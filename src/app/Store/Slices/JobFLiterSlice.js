...
export const FilterJobs = createAsyncThunk('FilterJobs', async (filterParams, { getState }) => {
    ...
    return allJobsData.job.jobs.filter(e => e.title.toLowerCase().includes(filterParams.query.title.toLowerCase()) && e.location.includes((filterParams.query.location)))
})
